...
| `budget-save` | `netlify/functions/budget-save.ts` | Create or update one monthly budget for the current user/month; summary is calculated server-side | POST | active_user+ (`requireActiveUser`) | `monthly_budgets` owner-scoped by `user_id` | budget planner | CORE | keep |
| `budget-list` | `netlify/functions/budget-list.ts` | List lightweight saved monthly budgets for the current user | GET | active_user+ (`requireActiveUser`) | `monthly_budgets` owner-scoped by `user_id` | budget planner | CORE | keep |
| `budget-get` | `netlify/functions/budget-get.ts` | Fetch one full monthly budget only when owned by the current user | GET | active_user+ (`requireActiveUser`) | `monthly_budgets` owner-scoped by `user_id` | budget planner | CORE | keep |
| `budget-delete` | `netlify/functions/budget-delete.ts` | Delete one monthly budget only when owned by the current user | POST/DELETE | active_user+ (`requireActiveUser`) | `monthly_budgets` owner-scoped by `user_id` | budget planner | CORE | keep |
| `budget-duplicate` | `netlify/functions/budget-duplicate.ts` | Duplicate an owned budget into a target month, copying planned amounts and resetting actual amounts to zero | POST | active_user+ (`requireActiveUser`) | `monthly_budgets` owner-scoped by `user_id` | budget planner | CORE | keep |
...
| `dividend-yield-lookup` | `netlify/functions/dividend-yield-lookup.ts` | Look up dividend yield through the internal DB cache, refreshing from MASSIVE or Alpha Vantage server-side when stale/missing | GET | premium_user/advisor/admin/superadmin | dividend_yield_cache | dividend reinvestment calculator | PREMIUM_TOOL | keep |
...
- Market data provider selection is server-side through `MARKET_DATA_PROVIDER`. Supported values are `alpha_vantage` and `massive`; when omitted, MASSIVE is preferred if `MASSIVE_API_KEY` exists, otherwise Alpha Vantage is used if `ALPHA_VANTAGE_API_KEY` exists.
- `ALPHA_VANTAGE_API_KEY` remains supported as fallback. `MASSIVE_API_KEY` enables MASSIVE. If the preferred provider fails and a fallback key is configured, provider refresh may fall back before using stale cache or returning a friendly unavailable message. If neither key is configured, dividend yield lookup returns a manual-entry configuration message.
