...
| `/app/tools/rent-room` | `app/(workspace)/app/tools/rent-room/page.tsx` | Legacy alias for room-rental scenario tool with saved scenario recall/edit | SCENARIO | active_user+ | `rent-room-list`, `rent-room-get`, `rent-room-save`, `rent-room-delete` | `RentRoomTool` | hide from nav (alias) |
| `/app/tools/rent-a-room` | `app/(workspace)/app/tools/rent-a-room/page.tsx` | Rent a Room Scenario tool with saved scenario recall/edit | SCENARIO | active_user+ | `rent-room-list`, `rent-room-get`, `rent-room-save`, `rent-room-delete` | `RentRoomTool` | keep as canonical label |
...
| `/app/budget-planner` | `app/(workspace)/app/budget-planner/page.tsx` | Manual monthly budget planner with income, planned expenses, actual spending, summaries, saved budgets, duplicate, delete, and print/PDF output | USER_CORE | active_user+ (`user`, `premium_user`, `advisor`, `admin`, `superadmin`) | `budget-list`, `budget-get`, `budget-save`, `budget-delete`, `budget-duplicate` | budget planner page + `lib/finance/budget-planner.ts` | keep as core MVP; future dashboard cash-flow integration optional |
