export function getMissingBankFields(bundle: Record<string, any>) {
  ...
  const miss = (v: any) => v === null || v === undefined || String(v).trim() === "";
  return {
    Personal: ["customer_name","date_of_birth","nationality","household_status","dependents","citizenship_status"].filter((k)=>miss(p[k])),
    ...
    Documents: ["has_id","has_proof_of_address","has_payslips","has_employment_letter","has_bank_statements","has_debt_statements","has_credit_report","has_purchase_agreement","has_valuation","has_business_financials","has_tax_returns"].filter((k)=>!Boolean(p[k]))
  };
}
