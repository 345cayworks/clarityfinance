...
type RoleFilter = "all" | "user" | "premium_user" | "advisor" | "admin" | "superadmin";
...
export default function Page() {
  ...
  return <div className="space-y-6">
    ...
    <div className="card">
      ...
      {!loading && tab === "users" && (filteredUsers.length ? <div className="space-y-2"><div className="flex gap-2 text-sm"><select className="rounded border px-2 py-1" value={roleFilter} onChange={(e)=>setRoleFilter(e.target.value as RoleFilter)}><option value="all">All Roles</option>{availableRoleOptions.map((r)=><option key={r.value} value={r.value}>{r.label}</option>)}</select><select className="rounded border px-2 py-1" value={userFilter} onChange={(e)=>setUserFilter(e.target.value as UserFilter)}><option value="all">All Activity</option><option value="pending">Pending Users</option><option value="recently_active">Recently Active</option><option value="recently_inactive">Recently Inactive</option></select></div>{filteredUsers.map((u)=>{const selectedRole=roleDrafts[u.id]||u.role||"user";const isPrimary=u.email.toLowerCase()===PRIMARY_ADMIN_EMAIL;const roleLocked=isPrimary;return <div key={u.id} className="rounded border p-3"><p className="font-medium">{u.name || "Unnamed user"} {isPrimary ? <span className="ml-2 rounded bg-indigo-100 px-2 py-0.5 text-xs font-semibold text-indigo-800">Superadmin</span> : null}</p><p className="text-sm text-slate-500">{u.email}</p><div className="mt-2 flex gap-2">{statusBadge(u.account_status,"account")}{statusBadge(u.role || "user","role")}</div><div className="mt-2 flex gap-2"><button className="rounded border px-2 py-1 disabled:opacity-50" disabled={isPrimary} onClick={()=>act("admin-user-deactivate",{userId:u.id})}>Deactivate</button><select className="rounded border px-2 py-1 disabled:opacity-50" disabled={roleLocked} value={selectedRole} onChange={(e)=>setRoleDrafts((p)=>({...p,[u.id]:e.target.value}))}>{availableRoleOptions.map((r)=><option key={r.value} value={r.value}>{r.label}</option>)}</select><button className="rounded border px-2 py-1 disabled:opacity-50" disabled={roleLocked || selectedRole===(u.role||"user")} onClick={async()=>{const ok=await act("admin-user-role-update",{userId:u.id,role:selectedRole},false); if(ok){setUsers((prev)=>prev.map((it)=>it.id===u.id?{...it,role:selectedRole}:it));}}}>Update Role</button></div></div>;})}</div> : <EmptyState title={roleFilter==="premium_user"?"No Premium Users found.":userFilter==="recently_inactive"?"No Recently Inactive Users found.":"No active users"} helper="Try a different filter." />)}
