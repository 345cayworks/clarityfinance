...
export default function ActionPlanPage() {
  ...
  const steps = useMemo(() => {
    ...
    if (fundMonths < 3) {
      ...
      plan.push(`Build emergency savings toward ${toCurrency(targetEmergencyFund, currency)} (about 3 months of expenses).`);
    }
    ...
    if (goal === "Cash-out refinance") {
      ...
      plan.push(`Estimate home value (current estimate: ${toCurrency(homeValue, currency)}).`);
      plan.push(`Confirm current mortgage balance (saved balance: ${toCurrency(mortgageBalance, currency)}).`);
      plan.push(`Calculate available equity (estimated: ${toCurrency(equity, currency)}).`);
    ...
    if (surplus > 0) {
      plan.push(`Auto-transfer ${toCurrency(Math.max(100, surplus * 0.4), currency)} monthly toward your top goal.`);
    }
      ...
      {steps.length === 0 ? (
        <div className="card text-center py-8">
          <p className="text-sm text-slate-600">
            Complete your profile to generate a personalised action plan.
          </p>
          <Link href="/app/onboarding"
            className="mt-4 inline-flex rounded-lg bg-[#0A2540] px-4 py-2
