...
export default function ReportsPage() {
  ...
  const baseSetupCost = getScenarioNumber(rentRoomResult, ["baseSetupCost"]);
  const contingencyAmount = getScenarioNumber(rentRoomResult, ["contingencyAmount"]);
  const totalSetupCost = getScenarioNumber(rentRoomResult, ["totalSetupCost"]);
  const effectiveMonthlyRent = getScenarioNumber(rentRoomResult, ["effectiveMonthlyRentOnceOccupied", "effectiveMonthlyRent", "monthlyRent"]);
  const monthlyAddedCosts = getScenarioNumber(rentRoomResult, ["monthlyAddedCosts"]);
  const netMonthlyProfit = getScenarioNumber(rentRoomResult, ["netMonthlyProfitOnceOccupied", "netMonthlyProfit", "monthlyNetProfit", "netMonthly"]);
  const operatingBreakEvenMonths = getScenarioNumber(rentRoomResult, ["operatingBreakEvenMonths", "breakEvenMonths"]);
  const calendarBreakEvenMonths = getScenarioNumber(rentRoomResult, ["calendarBreakEvenMonths"]);
  const activeRentalMonthsFirstYear = getScenarioNumber(rentRoomResult, ["activeRentalMonthsFirstYear"]);
  const firstYearGrossIncome = getScenarioNumber(rentRoomResult, ["firstYearGrossIncome"]);
  const firstYearAddedCosts = getScenarioNumber(rentRoomResult, ["firstYearAddedCosts"]);
  const firstYearOperatingProfit = getScenarioNumber(rentRoomResult, ["firstYearOperatingProfit"]);
  const firstYearNet = getScenarioNumber(rentRoomResult, ["firstYearNetAfterSetup", "firstYearNet"]);
  const annualProfitAfterBreakEven = getScenarioNumber(rentRoomResult, ["annualRecurringProfitAfterBreakEven", "annualProfitAfterBreakEven"]);
  ...
  return (
    <div className="space-y-4 print:bg-white">
      ...
      {activeReport === "rent-room" ? <section className="card space-y-3"><div className="flex flex-wrap items-start justify-between gap-3"><h2 className="text-lg font-semibold text-[#0A2540]">Rent a Room Scenario Report</h2><Link href="/app/tools/rent-a-room" className="print:hidden rounded-lg border border-slate-300 px-3 py-2 text-sm font-medium text-slate-700 hover:border-slate-400">Update Scenario</Link></div>{rentRoomLoading ? <p className="text-sm text-slate-600">Loading rent-a-room scenario…</p> : rentRoomScenario ? <><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4"><ScenarioMetric label="Base setup cost" value={toCurrency(baseSetupCost, metrics.currency)} /><ScenarioMetric label="Contingency amount" value={toCurrency(contingencyAmount, metrics.currency)} /><ScenarioMetric label="Total setup cost" value={toCurrency(totalSetupCost, metrics.currency)} /><ScenarioMetric label="Effective monthly rent once occupied" value={toCurrency(effectiveMonthlyRent, metrics.currency)} /><ScenarioMetric label="Monthly added costs" value={toCurrency(monthlyAddedCosts || rentRoomCostsTotal, metrics.currency)} /><ScenarioMetric label="Net monthly profit once occupied" value={toCurrency(netMonthlyProfit, metrics.currency)} /><ScenarioMetric label="Operating break-even" value={formatBreakEven(operatingBreakEvenMonths)} /><ScenarioMetric label="Calendar break-even including tenant search" value={formatBreakEven(calendarBreakEvenMonths)} /><ScenarioMetric label="Active rental months in first year" value={`${activeRentalMonthsFirstYear.toFixed(1)} months`} /><ScenarioMetric label="First-year gross income" value={toCurrency(firstYearGrossIncome, metrics.currency)} /><ScenarioMetric label="First-year added costs" value={toCurrency(firstYearAddedCosts, metrics.currency)} /><ScenarioMetric label="First-year operating profit" value={toCurrency(firstYearOperatingProfit, metrics.currency)} /><ScenarioMetric label="First-year net after setup" value={toCurrency(firstYearNet, metrics.currency)} /><ScenarioMetric label="Annual recurring profit after break-even" value={toCurrency(annualProfitAfterBreakEven, metrics.currency)} /><ScenarioMetric label="Security deposit collected" value={toCurrency(securityDepositCollected, metrics.currency)} note="Shown for cash-on-hand planning; not counted as profit because it may be refundable." /></div><div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700"><p className="font-medium text-[#0A2540]">Scenario status</p><p className="mt-1">{String(rentRoomResult.statusLabel ?? "Review the saved scenario assumptions.")}</p><p className="mt-1 text-xs text-slate-500">Security deposit assumption: not counted as profitability by default.</p><p className="mt-1 text-xs text-slate-500">Last updated: {new Date(rentRoomScenario.updated_at).toLocaleString()}</p></div></> : <p className="text-sm text-slate-600">No rent-a-room scenario found. Use the Rent-a-Room tool to create one.</p>}</section> : null}
