  ...
  if (normalizedEmail === ADMIN_EMAIL) {
    await sql`
      ...
      WHERE LOWER(email) = LOWER(${ADMIN_EMAIL})
        AND (role IS DISTINCT FROM 'superadmin' OR approval_status IS DISTINCT FROM 'approved' OR account_status IS DISTINCT FROM 'active')
    `;
    return { ok: true, identityUser };
  }

  const rows = (await sql`SELECT role FROM users WHERE email = ${normalizedEmail} LIMIT 1`) as RoleRow[];
  if (!["admin", "superadmin"].includes(rows[0]?.role ?? "")) {
